...
AeroCalc::AeroCalc(PlaneConfig config)
{
	// Length measurements are in millimeters and weight is in grams
	// Wing calculations
	wingspan = config.wingspan;	// Airplane wingspan
	weight = config.weight;	// Airplane weight
	wingRootChord = wingspan * config.wingRootChordModifier;	// Wing root chord length. Default modifier value is 16.67% of wingspan
	wingTipChord = wingRootChord * config.wingTipChordModifier;	//Wing tip chord length. Default modifier value is 100% of wing root chord
	...
	if (config.aileronType == "-s" || config.aileronType == "s")
	...
	hStabArea = config.hStabAreaModifier * wingSurfArea;	// Horizontal stabilizer area. Default modifier value is 22.5% of wing area
	...
	hStabTipChord = hStabRootChord * config.hStabTipChordModifier;	// Horizontal stabilizer tip chord. The modifier value can used to taper the horizontal stabilizer
	...
	fuseLen = wingspan * config.fuseLenModifier;	// Fuselage length. Default modifier value is 70% of wingspan
	noseLen = (fuseLen * config.noseLenModifier);	// Nose length. Default modifier value is 21% of fuselage length 
	...
	vStabArea = config.vStabAreaModifier * hStabArea;	// Vertical stabilizer area. Default modifier value is 50% of horizontal stab area
